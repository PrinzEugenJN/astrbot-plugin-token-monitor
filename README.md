...
> 上下文上限**自动探测**（用户设置 / 模型默认优先，平台兜底） · 压缩阈值 82% · 警告阈值 75%
...
- **上下文限制自动检测**：跟随 AstrBot 配置，用户显式设置优先、模型默认兜底、换模型自动跟，不写死任何模型值
- **旧会话自动标识**：自动标记被取代的旧会话，面板置灰沉底，一眼看清
...
| `context_limit` | 上下文上限（留空自动按模型/兜底值检测，非必填） | `0` |
| `stale_days` | 会话冻结多少天后标记为旧会话（0=不标记） | `30` |
